"""Tests for ``model_from_json_schema`` — JSON-schema-dict structured output.

Covers the conversion subset, the two CaCL footguns (omitted optionals
serialising as ``null``; title-less schemas), a clear failure on an
unsupported construct, and a full round-trip through ``structured_llm_call``
against a faked transport (patching ``llmkit._litellm.acompletion_structured``,
the same seam the other offline tests use).
"""

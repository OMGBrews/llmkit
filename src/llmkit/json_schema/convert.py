"""The *input* side: a JSON-schema dict in, a pydantic model class out.

:func:`model_from_json_schema` is an entry-point docstring wrapped around
:class:`_Converter`, so the two stay together — as do the three interdependent
keyword frozensets, one of which is derived from the other two at import.

Annotation-building and object-building are mutually recursive (that is how
nested objects and arrays-of-objects are built), so they are not separable
without injecting one side as a callback; they stay in one module deliberately.
"""
...
from llmkit.json_schema.emitted import JsonDict, JsonSchemaModel, JsonSchemaModelAllow
...
class _Converter:
    ...
    def _build_object_uncached(
        ...
        base: type[JsonSchemaModel] = (
            JsonSchemaModelAllow if additional is True else JsonSchemaModel
        )
